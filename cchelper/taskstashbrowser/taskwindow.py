...
from .taskwindows_ui import Ui_TaskWindow
from .taskcomposer import TaskComposer
...
class TaskWindow(QMainWindow, Ui_TaskWindow):
    ...
    def __init__(self, parent: QWidget = None) -> None:
        ...
        self.view.doubleClicked.connect(self.edit_task)
        ...
        self.task: Task = None
        self.taskcomposer: TaskComposer = None

    def edit_task(self, idx):
        assert idx.isValid()
        if self.taskcomposer is not None:
            self.taskcomposer.close()
        self.taskcomposer = TaskComposer(self)
        self.taskcomposer.set_model(self.model, idx)
        self.taskcomposer.show()
        self.hide()
    ...
    def refresh_tasks(self):
        ...
        self.arxivButton.setEnabled(T)
        self.delButton.setEnabled(T)
        if not self.tasks:
            self.arxivButton.setEnabled(F)
            self.delButton.setEnabled(F)
